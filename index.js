...
async function startSession(sessionName, isManual = false) {
    ...
    // ▼▼▼▼▼▼ CAMUFLAJE STEALTH ACTIVADO ▼▼▼▼▼▼
    const puppeteerConfig = {
        headless: true,
        protocolTimeout: 300000, 
        // ESTO ES LO NUEVO: Esconde que es un robot
    ...
    client.on('ready', () => { 
        ...
        // --- HEARTBEAT SEGURO (Mantiene vivo pero oculto) ---
        if (heartbeatInterval) clearInterval(heartbeatInterval);
        heartbeatInterval = setInterval(async () => {
            try {
                // Solo si es horario laboral mantenemos el latido fuerte
                const office = checkOfficeHours();
                if(office.isOpen) {
                    await client.getState(); 
                    console.log('💓 Heartbeat: (Horario Laboral) Conexión OK');
                } else {
                    console.log('🌙 Heartbeat: (Modo Noche) Silencio...');
                }
            } catch (e) {}
        }, 300000); 
